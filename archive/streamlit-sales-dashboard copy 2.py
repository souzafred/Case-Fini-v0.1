...
with tab1:
    if sell_in is None or sell_out is None:
        ...
        c3.metric("Ticket Médio", format_currency(ticket_in))
        c4.metric("Crescimento", f"{growth_in:.1f}%", delta=f"{growth_in:.1f}%")

        st.markdown("### Sell Out")
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("Total", format_currency(total_out))
        c2.metric("Volume (Unid)", f"{volume_out:,.0f}".replace(",","."))
        c3.metric("Ticket Médio", format_currency(ticket_out))
        c4.metric("Crescimento", f"{growth_out:.1f}%", delta=f"{growth_out:.1f}%")
        ...
        with a1:
            ...
            fig.update_layout(
                ...
                yaxis=dict(range=[0,150],ticksuffix='%'), hovermode='x'
            )
        ...
        with a2:
            fig=go.Figure()
            fig.add_trace(go.Scatter(
                x=dfm['M'], y=dfm['Valor_Total'],
                mode='lines+markers+text',
                text=[f"R$ {v:,.0f}" for v in dfm['Valor_Total']],
                textposition="top center",
                line=dict(color='#8884d8',width=2)
            ))
            fig.add_trace(go.Scatter(
                x=dfm['M'], y=dfm['Valor_SellThrough'],
                mode='lines+markers+text',
                text=[f"R$ {v:,.0f}" for v in dfm['Valor_SellThrough']],
                textposition="bottom center",
                line=dict(color='#82ca9d',width=2)
            ))
            fig.update_layout(
                title="Sell In vs Sell Out", hovermode='x'
            )
            st.plotly_chart(fig, use_container_width=True)
